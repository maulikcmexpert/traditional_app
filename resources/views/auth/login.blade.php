<x-guest-layout>
    ...
    <form method="POST" id="loginForm" action="{{ route('login') }}" class="loginForm">
        @csrf

        <!-- Email Address -->
        <div>
            <x-input-label for="email" :value="__('Email')" />
            <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email')" autofocus autocomplete="username" style="border:1px solid #6366f1" />
        ...
        <div class="mt-4">
            ...
            <x-text-input id="password" class="block mt-1 w-full" style="border:1px solid #6366f1" type="password" name="password" autocomplete="current-password" />
